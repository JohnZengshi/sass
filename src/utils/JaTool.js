...
export const JaTools = {
    ...
    transformationDataToHtml(page) {
    	...
    	for(let data of list) {
    		...
    		if(data.componnent =="charCode"){//条码
            ...
            if(data.componnent =="barcode2"){//二维码
                    let url = data.codeUrl + '&barcode=' + data.sample + '&vcode=' + JaTools.getCode(list);
                box = $("<img class='jatools-coder' src='http://127.0.0.1:31227/api?type=coder&code=" + url + "&style=type:qr;&width=200&height=200'>");
            }
    		box.css({
