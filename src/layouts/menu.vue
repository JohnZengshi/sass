        ...
        <template v-for="(menu,i) in systemMenu">
            <div class="nav" :class="sysShow == i ? 'active' : ''" v-for="(tl,p) in rootList" v-if="tl.mainName==menu.text">
                ...
                <template v-if="menu.children.length > 0 ">
                    <router-link tag="h3" :to="menu.path" :data-text="menu.text" class="root" :class="systemItemClass.rootIndex == i ?
              (menu.path.length > 0 ? 'active path-href' : 'active') : (menu.path.length > 0 ? 'path-href' : '')" @click.native="systemTab(i,menu.path.length > 0,menu)">
                        <i class="iconfont" :class="'icon-'+menu.icon"></i>
                        <!--<i v-if="menu.children.length > 0 " class="el-icon el-icon-arrow-down"></i>-->
                    </router-link>
                    <!--二级菜单-->
                    <ul class="item secondary_item" :class="menu.path.length > 0 ? 'path-href' : ''" v-if="menu.children.length > 0 " :data-text="menu.text">

                        <!--一级菜单有链接的情况下，二级菜单有多个菜单的处理-->
                        <template v-if="menu.path.length > 0">
                            <router-link tag="li" :to="menu.path" class="path-target" @click.native="systemItem(i,-1, menu)">{{menu.text}}
                            </router-link>
                        </template>

                        <template v-for="(item,b) in menu.children">
                            <router-link tag="li" :to="item.path" :class="systemItemClass.rootIndex == i && systemItemClass.childIndex == b ? 'active' : ''" v-for="api in applyLists" @click.native="systemItem(i, b, menu,item)" v-if="api.applyName.indexOf(menu.text) >= 0 && getPrivilegeList(api.applyName, item.text)">
                ...
                <template v-else-if="menu.children.length == 0 ">
                    <router-link tag="h3" :to="menu.path" class="root not-item" :class="systemItemClass.rootIndex == i ?
              (menu.path.length > 0 ? 'active path-href' : 'active') : (menu.path.length > 0 ? 'path-href' : '')" :data-text="menu.text" @click.native="systemTab(i,menu.path.length > 0,menu)">
                        <i class="iconfont" :class="'icon-'+menu.icon"></i>
                    </router-link>
                    <ul class="item menu-item">
                        <router-link tag="li" :to="menu.path" @click.native="systemItem(i,0, menu)">
                            {{menu.text}}
                        </router-link>
                    </ul>
                </template>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            bus.$on('menuTabClick', function (data) {
        ...
        methods: {
            ...
            systemTab(loop, type, menu) {
                ...
                if (this.myMenuTab.length == 0) {
                    ...
                    for (let i = 0; i < this.myMenuTab.length; i++) {
                        if (this.myMenuTab[i].menu.path == menu.path || menu.path == '') {
                            pushStatus = false
                        }
                    }
                    if (pushStatus) {
                ...
                this.myMenuTab && bus.$emit('menuTabDataChange', this.myMenuTab) //发送事件
                menu && menu.path && bus.$emit('menuTabPath', menu.path)
            },
            ...
            systemShow() {
                ...
                if (!this.isFixed) {
                    ...
                    setTimeout(function () {
                        if (bodyHeader) bodyHeader.style.left = document.getElementsByClassName('body-box')[0].getBoundingClientRect()
                            .left + 'px'
                        if (leftTittle) leftTittle.style.left = document.getElementsByClassName('body-box')[0].getBoundingClientRect()
                            .left + 'px'
                        if (totalTable) totalTable.style.left = document.getElementsByClassName('body-box')[0].getBoundingClientRect()
                            .left + 'px'
                        if (bodyRow5) bodyRow5.style.left = document.getElementsByClassName('body-box')[0].getBoundingClientRect()
                            .left + 'px'
                    }, 300)
            ...
            //子菜单点击事件
            systemItem(rootIndex, childIndex, menu, item) {
                /*
                ...
                if (this.myMenuTab.length == 0) {
                    ...
                    for (let i = 0; i < this.myMenuTab.length; i++) {
                        ...
                        if (this.myMenuTab[i].menu.path == menu.children[childIndex].path || menu.children[childIndex].path == '') {
                            pushStatus = false
                        }
                    }
                    if (pushStatus) {
