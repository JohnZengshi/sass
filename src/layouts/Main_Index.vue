  ...
  methods: {
    ...
    switchSkin(item, index) {
      ...
      localStorage.setItem(
        "bgUrl",
        JSON.stringify({ url: item.bgUrl, flag: item.flag, index: index })
      );
      if (item.flag == "custom") {
        $("#body").addClass("body-shield");
    ...
    getNoticeList() {
      ...
      seekNoticeList(options).then(
        res => {
          this.noticeList = res.data.data.dataList;
          console.log(res);
          for (let i = 0; i < this.noticeList.length; i++) {
            if (this.noticeList[i].createTime) {
              let year = this.noticeList[i].createTime
                .toString()
                .substring(0, 4);
              let month = this.noticeList[i].createTime
                .toString()
                .substring(4, 6);
              let data = this.noticeList[i].createTime
                .toString()
                .substring(6, 8);
              let hour = this.noticeList[i].createTime
                .toString()
                .substring(8, 10);
              let time = this.noticeList[i].createTime
                .toString()
                .substring(10, 12);
              this.noticeList[i].createTime =
      ...
      seekGetUserInfo({
        ...
        res => {
          ...
          if (res.body.state) {
            ...
            this.currentShop =
              this.shopList.length > 0 ? this.shopList[0] : this.currentShop;
          } else {
            this.$message({
              type: "error",
