    ...
    <div ref="switch_skin" class="switch-skin">
      ...
      <div class="row-3">
        ...
        <div class="notice row3-block">
          <h3>公告列表<span @click="goNotict">更多</span></h3>
          <ul class="notice-list">
            <div>
              <li v-for="(item, index) in noticeList" v-if="item.noticeType == 1" :key="index">
                <div class="notice-title" @click="goNotict"><span class="iconfont icon-gonggao"></span>{{item.title}}</div>
...
export default {
  ...
  methods: {
    ...
    switchSkin (item, index) {
        ...
        localStorage.setItem('bgUrl', JSON.stringify({url: item.bgUrl, flag: item.flag, index: index}))
        if (item.flag == 'custom') {
            $('#body').addClass('body-shield')
    ...
    goUnder (item) { // 去到单据详情页面
      switch (item.orderType) {
        case '入库':
          this.$router.push({path: '/work/storage/detail', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '退库':
          this.$router.push({path: '/work/storageReturn/NewStorageReturn', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '调库':
          this.$router.push({path: '/work/transferStorage/newTransferStorage', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '调柜':
          this.$router.push({path: '/work/transferCabinet/newTransferCabinet', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '修改':
          this.$router.push({path: '/work/amend/index', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '发货':
          this.$router.push({path: '/work/sipping/newSipping', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '退货':
          this.$router.push({path: '/work/salesReturn/newSalesReturn', query: {
            orderNumber: item.orderNum
          }})
          break;
        case '销售':
          this.$router.push({path: '/work/sell/sellReceiptsList', query: {
            orderNumber: item.orderNum
          }})
          break;
      }
