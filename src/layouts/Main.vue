...
export default {
  ...
  methods: {
    ...
    _downloadTable() {
      ...
      downloadTable(options).then(res => {
    ...
    createdWebSocket(parm) {
      ...
      ws.onmessage = function(evt) {
        ...
        } else if (datas.msgType == "08") {
          // 人脸识别
          console.log("收到人脸的数据", evt);
          _self.faceWebsocked(datas);
        } else if (
          datas.msgType == "10" &&
          sessionStorage.getItem("tokenId") != datas.tokenId &&
          datas.os == "web" &&
          sessionStorage.getItem("id") == datas.userId
        ) {
          // 退出
          ws.close();
    ...
    establishWebSocket(parm) {
      ...
      let socketUrl =
        process.env.NODE_ENV === "development"
          ? "ws://uat.yunzhubao.com:8080/yunzhubao/ws/facepass/"
          : "wss://app.yunzhubao.com:9092/yunzhubao/ws/facepass/";
    ...
    scrollFun(el) {
      eventBus.$emit("scrollTop", {
        itemTop: el.target.scrollTop,
        diff: parseInt(
          el.target.children[0].clientHeight - el.target.clientHeight
        )
      });
    },
    userRoleList1() {
    ...
    SelectArea(parm) {
      ...
      this._operateUpdateCompany(
        4,
        `${parm.provId}-${parm.cityId}-${parm.areaId}`,
        true,
        parm
      );
    },
    cosImg(parm) {
      ...
      operateUpdateFaceUserImage(options).then(res => {
        if (res.data.state == 200) {
          this.$message({
            message: "修改成功",
            type: "success"
    ...
    _seekFaceUserImageList() {
      seekFaceUserImageList().then(res => {
        if (res.data.state == 200) {
    ...
    blueAndUpdataSex(type) {
      ...
      operateUpdateUser(options).then(
        res => {
          if (res.data.state == 200) {
            this.$message({
              message: "修改成功",
    ...
    blurAndUpdateUser(evt, type) {
      // 上传个人信息
      if (
        type === 3 &&
        !/^(\w)+(\.\w+)*@(\w)+((\.\w+)+)$/.test(evt.target.value)
      ) {
        this.$message({
      ...
      seekGetUserInfo(options).then(
        res => {
          ...
          for (let i = 0; i < this.userInfoData.roleList.length; i++) {
            if (
              this.userInfoData.roleList[i].role == 1 ||
              this.userInfoData.roleList[i].role == 2 ||
              this.userInfoData.roleList[i].role == 3
            ) {
