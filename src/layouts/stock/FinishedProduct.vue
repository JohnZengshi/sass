    ...
    <div class="finished-product-header">
      ...
      <div class="header-right">
        ...
        <ul>
          ...
          <li class="select-w80-h28" v-if="smallClassList.length > 0 ">
            ...
            <!--<el-select ref="shopLable" style="vertical-align: middle;" v-model="smallClass.id"  clearable placeholder="成色小类">
              <el-option     
                v-for="item in smallClassList"
                @click.native="_seekFinishedData"
                :label="item.classesName"
                :value="item.classesId">
              </el-option>
            </el-select>-->
...
import {getProductTypeList, seekProductClassList, seekFinishedData} from 'Api/commonality/seek.js'
            ...
            setTimeout(()=>{
              if (res.data.data.dataList[0]) {
                ...
                for (let i = 0; i < res.data.data.dataList.length; i++) {
                  this.chart_configData.dataList[i].weight = res.data.data.dataList[i].weight.toFixed(3)
                  this.chart_configData.dataList[i].price = res.data.data.dataList[i].price.toFixed(2)
                }
  ...
  .finished-product-header{
    ...
    .header-right{
      ...
      .class-btn{
        ...
        .actions{
          background-color: #ffb549;
          ///background: url('./../../assets/img/ok.png') no-repeat center center !important;
          img {
            display: block;
          }
        }
