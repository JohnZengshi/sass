...
export default {
  ...
  computed: {
    ...
    showNewAddData: function() {
      ...
      if (this.endGetData > 0) {
        // 后台最后一页数据不满的情况下
        if (currentPage === endGetPage) {
          // 是后台最后一页
          ...
          // 大于后台最后一页
          let startNum =
            (currentPage - endGetPage - 1) * 10 + (10 - endGetData);
    ...
    isMakeOrderManId() {
      return (
        this.receiptsIntroList.makeOrderManId == sessionStorage.getItem("id")
      );
    },
    printOptions() {
      // 打印单据
      ...
      if (isUser) {
        // 制单人
        return true;
      } else if (isCheckOrderMan) {
        // 审核人
        if (this.receiptsIntroList.checkType === "2") {
          // 审核中
          return true;
        }
      } else {
  ...
  methods: {
    ...
    settingUserRole() {
      ...
      seekSettingUserRole(options).then(
        res => {
          console.log(res);
    ...
    dropReturn(data) {
      ...
      operateHandleXGReceipt(options).then(
        res => {
          console.log(res);
    ...
    delOrder() {
      if (this.nowStatus == 2) {
        ...
        this.$confirm("此操作将永久删除该单据, 是否继续?", "提示", {
          ...
          .then(() => {
            operateDeleteByOrderNumber(options).then(
              res => {
                console.log(res);
                if (res.data.state == 200) {
    ...
    closeMask(type) {
      ...
      if (type == 2) {
        ...
        console.log(option);
        operateRemarkOperation(option).then(
          res => {
            console.log(res);
      ...
      if (type == 1) {
        ...
        this.$confirm("确定要撤回审核吗?", "提示", {
          ...
          .then(() => {
            ...
            operateReceiptOperation(options).then(
              res => {
                console.log(res);
                if (res.data.state == 200) {
                  this.$message({
        ...
        this.$confirm("确定要提交审核吗?", "提示", {
          ...
          .then(() => {
            ...
            operateSubmitAudit(options).then(
              res => {
                console.log(res);
                if (res.data.state == 200) {
                  this.$message({
    ...
    updateGoodsList() {
      ...
      getUpdateGoodsList(options).then(
        res => {
          console.log(res);
        },
        res => {
    ...
    handleCurrentChange(val) {
      ...
      if (this.newPage <= val) {
        // 最后一页才给选择
        if (this.datas.newGapDatas.length > 0) {
          // 有空白数据的情况下
          setTimeout(function() {
            _self.getFocus();
          }, 100);
    ...
    delProduct(barcode, index, operate, showData) {
      if (showData.barcode) {
        ...
        this.datas.newGapDatas.splice(
          index - this.datas.postDataList.length,
          1
        );
      }
    },
    queryBarCode(barcode, index, operate, el, keys = 0) {
      // 通过条码号操作数据
      if (barcode) {
        let options = {
          productList: [
        ...
        operateProductList(options).then(
          response => {
    ...
    sendlayLoad() {
      this.dataGridOptions.pageSize += 15;
      seekOutStorageData(this.dataGridOptions).then(
        res => {
          if (res.data.state == 200) {
    ...
    getproductTpye: function(parm) {
      // 商品属性
      return productTpyeState(parm);
    },
    getSeekSellReceiptsIntro() {
      // 单据简介
      let options = {
        orderNum: this.$route.query.orderNumber
      };
      seekReceiptTKSynopsis(options).then(
        response => {
          if (
            response.data.data.makeOrderManId == sessionStorage.getItem("id")
          ) {
            this.isMakeMan = true;
          } else {
          ...
          if (response.data.state === 200) {
            if (response.data.data.makeOrderManId != sessionStorage.id) {
              //非本人不可以操作
