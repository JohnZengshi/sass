<template>
  <div class="dg-container">
    ...
    <div class="datagrid-container" :class="{[!browserType ? 'animat-scroll-firefox' : 'animat-scroll']:slipPointer}" ref="datagrid"
      @scroll="watchScroll($event)" :style="'padding-left:'+ fixedFullSize +'px'">

      <!--左边固定表格区-->
      <dgridBody :seekBarcode="seekBarcode" :seekFlag="seekFlag" :dgDataList="dgDataList" :synopsiData="synopsiData" ref="dgridfixed"
        :orderNum="orderData.orderNum" :smallDataList="smallDataList" :selectContainer="selectContainer" @updataApi="updataApi"
        @updataData="updataData" @updataSelectContainer="updataSelectContainer" @updataEditApi="updataEditApi" @updateActiveIndex="updateActiveIndex"
        @updateActiveSelectOn="updateActiveSelectOn" :activeClassIndex="activeClassIndex" :fixedFullSize="fixedFullSize">
      </dgridBody>

      <!-- 加载更多未读数据 -->
      <ReadMoreData :allData="synopsiData" :dgDataList="dgDataList" ref="ReadMoreDataDmo" @readMoreData="readMoreData"></ReadMoreData>
    </div>

    <!--表尾-->
    <dgridfooter :smallDataList="smallDataList" ref="footer" :footerData="footerData" @setSynopsiData="updataSynopsiData" :orderNum="orderData.orderNum">
    </dgridfooter>
    ...
    methods: {
      ...
      getPrintData() {
        let res = getUpdateGoodsList({
          ...
          if (res.data.state == 200) {
            this.sellList_print = res.data.data;
            return true;
          } else {
            this.$message({
              type: 'error',
