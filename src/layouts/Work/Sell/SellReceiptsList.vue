        ...
        <div class="body-row2" :data-status="curStatus.statusName" :class="{'color1': nowStatus == 1, 'color2': nowStatus == 2,
                ...
                <div class="left-main">
                    ...
                    <div class="select-container">
                        ...
                        <div class="item shop">
                            ...
                            <select-drop ref="selectDrop" class="selected_dropdown" :titleName="receiptsIntroList.sellManName" dataType="销售人1" v-if="filterUserType" :propList="shopUserList" :isDrop="true" @dropReturn="dropReturn"></select-drop>
                            <span v-else>
                    ...
                    <div class="right-main-first">
                        <div class="right-main-item">
                            <div class="item-title">
                                {{receiptsIntroList.totalNum == '' || receiptsIntroList.totalNum == null ? 0 : receiptsIntroList.totalNum}} <span>件</span>
                            </div>
                            <div class="item-type">件数</div>
                        </div>
                        <div class="right-main-item">
                            <div class="item-title">
                                {{(Number(receiptsIntroList.totalWeight) || 0).toFixed(3)}} <span>g</span>
                            </div>
                            <div class="item-type">件重</div>
                        </div>
                        <div class="right-main-item">
                            <div class="item-title">
                                {{(Number(receiptsIntroList.price) || 0).toFixed(2)}} <span>元</span>
                            </div>
            ...
            <div class="cashierInfo">
                <div class="cash-tittle">
                    <span class="cashier">收银员:{{stepIndex == 1 ? '' : receiptsIntroList.cashUserName}}</span> <span class="time">{{cashierTime}}</span>
                </div>
                ...
                <div class="cash-footer">
                    <span class="total">合计<i>{{priceType.totalPrice}}<em>元</em></i></span> <span class="change">找零<i>{{priceType.change}}<em>元</em></i></span>
                </div>
            </div>

            <!-- 步骤条 -->
            <steps-path :isRemark="isRemark" :orderNum="$route.query.orderNumber" :statusREfresh="statusREfresh" @setStatus="getStatus"></steps-path>
            <!--备注-->
        ...
        <div class="body-row4">
            ...
            <div class="btn-list">
                <div class="btn" v-if="nowStatus != 6 && shopRole && receiptsIntroList.cashStatus == 2 || multipleIdentities == 'Y' && receiptsIntroList.cashStatus == 2" @click="selOperateType(1)">
                    <span class="iconfont icon-xiaoshou1"></span> <span>销售</span>
                </div>
                <div class="btn" v-if="nowStatus != 6 && companyPosition == 4 && receiptsIntroList.cashStatus == 2|| nowStatus != 6 && companyPosition == 5 && receiptsIntroList.cashStatus == 2 || multipleIdentities == 'Y' && receiptsIntroList.cashStatus == 2" @click="selOperateType(2)">
                    <span class="iconfont icon-huigou"></span> <span>回购</span>
                </div>
                <div class="btn" v-if="shopRole" @click="collectMoney">
                    <span class="iconfont icon-shouyindadan"></span> <span>收银打单</span>
                </div>
                <div class="btn" @click="exportTab()">
                    <span class="iconfont icon-daochu"></span> <span>导出表格</span>
                </div>
                <div class="btn" @click="tabPrin()">
                    <span class="iconfont icon-dayin"></span> <span>打印单据</span>
                </div>
                <div class="btn" @click="remarkDialog = true">
                    <span class="iconfont icon-beizhu"></span> <span>备注</span>
                </div>
                <div class="btn" @click="delOrder" v-if="isCanDelCurrentInvoices">
                    <span class="iconfont icon-shanchu1"></span> <span>删除</span>
                </div>
            </div>
        </div>
            ...
            <div class="cashier-body">
                ...
                <div class="cashier-footer">
                    ...
                    <el-select class="template-selector" :disabled="isPrintOnly == false" v-model="templateId" :placeholder="placeText" style="float:right">
                        <el-option :label="qualityTemplate.templateName" :value="qualityTemplate.templateId" v-for="(qualityTemplate,index) in qualityTemplateList" :key="index"></el-option>
                    </el-select>
...
<div>
    ...
    <el-dialog :visible.sync="isShowDio" custom-class="dialogDom" element-loading-text="拼命加载中">
        ...
        <div class="block-class block-0" v-if="tep == 1">
            ...
            <div class="list-wrap first-wrap">
                <div class="btn-wrap">
                    <i @click="attribute(1)" v-if="operateTypeNum != 2 && selWay == 1">成品</i> <i @click="attribute(2)">旧料</i>
                </div>
            </div>
        ...
        <div class="block-class block-1" v-if="tep == 2">
            ...
            <div class="list-wrap">
                ...
                <ul class="right-list">
                    <li :class="{active: entry.tep1List.rightClassId == item.classesId}" v-for="(item, index) in entry.tep1List.rightClassList" :key="index " @click="selBigClassId(item, 1)">{{item.classesName}}</li>
                </ul>
            </div>
            <div class="footer">
                <div class="footer-right">
                    <span @click.stop="goPro" v-if="entry.tep1List.isShowBtn">上一步</span> <span @click.stop="goNext" v-if="entry.tep1List.isShowBtn1">下一步</span>
                </div>
            </div>
        </div>
        <!--  成色大类 -->
        <div class="block-class block-2" v-if="tep == 3">
            ...
            <div class="list-wrap">
                ...
                <ul class="right-list">
                    <li v-for="(item, index) in entry.tep2List.rightClassList" :key="index" @click="selBigClassId(item, 2)">{{item.classesName}}</li>
                </ul>
            </div>
            <div class="footer">
                ...
                <div class="footer-right">
                    <span @click.stop="goNext" v-if="entry.tep2List.isShowBtn1">下一步</span> <span @click.stop="goNext" v-if="isSkip == false">跳过</span>
                </div>
        ...
        <div class="block-class block-3" v-if="tep == 4">
            ...
            <div class="list-wrap">
                ...
                <ul class="right-list">
                    <li v-for="(item, index) in entry.tep3List.rightClassList" :key="index" @click="selBigClassId(item, 3)">{{item.classesName}}</li>
                </ul>
            </div>
            <div class="footer">
                <div class="footer-left">
                ...
                <div class="footer-right">
                    <span @click.stop="goNext" v-if="entry.tep3List.isShowBtn1">下一步</span> <span @click.stop="goNext" v-if="isSkip == false">跳过</span>
                </div>
            </div>
        </div>
        <!--  首饰名称 -->
        <div class="block-class block-4" v-show="tep === 5" v-loading="isLoading">
            ...
            <div class="list-wrap">
                ...
                <ul class="right-list">
                    <li v-for="(item, index) in entry.tep4List.rightClassList" :key="index" @click="selBigClassId(item, 4)">{{item.classesName}}</li>
                </ul>
            </div>
            <div class="footer">
                <div class="footer-right">
    ...
    <!-- 批量添加 -->
    <BatchAddReceipts v-if="popup.batchAddReceiptsPopup" :isPopup="popup.batchAddReceiptsPopup" :supplierListData="supplierListData1" v-on:addReceiptsState="delectReceiptsState"></BatchAddReceipts>
    <!-- 备注弹窗 -->
    ...
    <!-- 新增会员 -->
    <addLeaguerDia v-if="addLeaguer" :addLeaguer="addLeaguer" :shopId="receiptsIntroList.shopId" :shopManageRole="shopManageRole" :shopManRole="shopManRole" :orderNum="receiptsIntroList.id" :memberId="receiptsIntroList.memberId" @closeReturn="closeReturn"></addLeaguerDia>
    <!-- 信息编辑页面 -->
    <el-dialog :visible.sync="editLeaguer" :modal="true" top="10%" customClass="member-info-dialog" :close-on-click-modal="false" @close="close">
...
<div ref="tablePrint" v-if="isPrint==1">
    <table-print :tabSwitch="tabSwitch" :reportType="getReportType()" :orderData="receiptsIntroList" :printSelectDate="printSelectDate" :receiptsIntroList="receiptsIntroList" :dataGridStorage="dataGridStorage"></table-print>
</div>
</div>
</div>
<!--质保单打印-->
<div id="windowPrintView" v-if="isPreview" ref="windowPrintView">
    <TemplatePreviewCanvasComponent :showEmpty="'N'" :isPrintCanvas="true" :canvas="canvas" :templateData="templateData" :class="i%2==0 ? 'printViewInside' : ''" v-for="(i,index) in pageNumber" :key="index" :page="i"></TemplatePreviewCanvasComponent>
</div>
...
<div style="display: none;">
    <sell-template title="销售" ref="sellTemplate" :isShowCost="isShowCost" :sellList="dataGridStorage" :headerData="receiptsIntroList"></sell-template>
...
//        "saveSuccessData", // 保存弹窗数据
    ...
    methods: {
        ...
        selBigClassId(item, type) { // 1 产品类别  2 成色大类  3 珠宝名称  4 首饰名称
            if (type == 1) {
                this.entry.tep1List.rightClassId = item.classesId;
                this.entry.tep1List.rightClassName = item.classesName;
                this.tep = 3
                console.log('entry.tep2List.classesList', this.entry.tep2List.classesList)
            } else if (type == 2) {
                this.entry.tep2List.rightClassId = item.classesId;
                this.entry.tep2List.rightClassName = item.classesName;
                this.tep = 4
            } else if (type == 3) {
                this.entry.tep3List.rightClassId = item.classesId;
                this.entry.tep3List.rightClassName = item.classesName;
                this.tep = 5
            } else if (type == 4) {
                this.entry.tep4List.rightClassId = item.classesId;
                this.entry.tep4List.rightClassName = item.classesName;
                if (this.operateTypeNum && this.isAllow) {
                    this.isAllow = false
        ...
        editProduct() {
            ...
            let data = {
                ...
                alterList: [
                    {
                        productTypeId: this.entry.tep1List.rightClassId,
                        productTypeName: this.entry.tep1List.rightClassName,
                        metalColor: this.entry.tep2List.rightClassName,
                        gemName: this.entry.tep3List.rightClassName,
                        jewelryName: this.entry.tep4List.rightClassName,
        ...
        //         if (!template.components[i].data.isNullPrint) {
        //           for (let j of json.data.baseInfoList) {
        //             if (template.components[i].data.propertyCode == j.key) {
        //               if (!j.value || j.value == '0.00' || j.value == '0.000') {
        //                 delete template.components[i]
        //                 break;
        //               }
        //             }
        //           }

        //           for (let j of json.data.productList[0].codeList) {
        //             if (template.components[i]) {
        //             	if (template.components[i].data.propertyCode) {
        //             		if (template.components[i].data.propertyCode == j.key) {
        //                  if (!j.value || j.value == '0.00' || j.value == '0.000') {
        //                    delete template.components[i]
        //                    break;
        //                  }
        //                }
        //             	} else if (template.components[i].data.children) {
        //             		for (let k in template.components[i].data.children) {
        //             			if (!template.components[i].data.children[k].data.isNullPrint) {
        //              			if (template.components[i].data.children[k].data.propertyCode == j.key) {
        //                    if (!j.value || j.value == '0.00' || j.value == '0.000') {
        //                      delete template.components[i].data.children[k]
        //                      break;
        //                    }
        //                  }
        //             			}
        //             			if (template.components[i].data.children[k]) {
        //             				if (template.components[i].data.children[k].data) {
        //              				for (let G in template.components[i].data.children[k].data.children) {
        //              					// if (template.components[i].data.children[k].data.children[G].data.children) {
        //              						if (!template.components[i].data.children[k].data.children[G].data.isNullPrint) {
        //                  			if (template.components[i].data.children[k].data.children[G].data.propertyCode == j.key) {
        // 	                      if (!j.value || j.value == '0.00' || j.value == '0.000') {
        // 	                        delete template.components[i].data.children[k].data.children[G]
        // 	                        break;
        // 	                      }
        // 	                    }
        //                 			}
        ...
        //       }
        //       let newDatas = []
        //       for (let i of template.components) {
        //         if (i) {
        //         	if (i.data.children) {
        //         		let children = []
        //         		for (let j of i.data.children) {
        //         			if (j) {
        //         				children.push(j)
        //          			if (j.data) {
        //           			if (j.data.children) {
        //            			let childrenT = []
        //            			for (let G of j.data.children) {
        //            				if (G) {
        //            					childrenT.push(G)
        //            				}
        //            			}
        //            			j.data.children = childrenT
        //           			}
        //          			}
        //         			}
        //         		}
        //         		i.data.children = children
        //         	}
        //         	newDatas.push(i)
        //         }
        //       }
        //       template.components = newDatas
        //       this.canvas = template
        //       this.windowPrint(template, json.data)
        //     }else {
        //       this.$message({
       ...
       */
        ...
        resetList(data) {
            this.goodsTypeList.recycleList = data.recycleList;
            this.goodsTypeList.saleList = [];
            this.goodsTypeList.exchangeList = [];
            this.goodsTypeList.relationList = [];
            this.saleList = data.saleList;
            let saleList = [];
            for (let item of data.exchangeList) {
                if (item.isRelation === 'Y') {
                    ...
                    for (let sale of item.relationList) {
                        ...
                        if (list) {
                            isRelation = true;
                            item.totalGoldWeight = +list.goldWeight + +item.totalGoldWeight;
                            item.totalPrice = +list.price + +item.totalPrice;
                            saleList.push(list);
                        }
                    }
                    if (isRelation) {
                        saleList.push(item);
                        this.goodsTypeList.relationList.push(item);
                    }

                } else {
                    this.goodsTypeList.exchangeList.push(item);
                }
            }
