...
<table class="table-print-wrap" ref="tableContainer" v-if="reportType == 1">
	<div>
		<tr class="tb-tr" v-for="(tb,index) in tempArray">
			...
			><div class="word-wrap-td" :style="tableCell(tab.width)" v-text = "tab.childType == ''? (index+1)  : tab.toFixed ? toFixed(tb[tab.childType],tab.countCut) : tb[tab.childType]"></div></td>
		</tr>
	...
	<div>
		<div class="tb-category" v-for="(caty, ind) in dataGridStorage.dataList" :index="resetIndex(ind)">
			...
			<tr class="tb-total" style="background:#e9f4fe;" v-if="positionSwitch"> <!-- 位置小计 -->
				...
				>
				<div class="word-wrap-td" :style="tableCell(tab.width)" v-html = "f == 1 ? '<b>小计</b>' : tab.toFixed ? toFixed(caty[tab.totalType0], tab.countCut) : caty[tab.totalType0]"></div>
				</td>
			</tr>
		</div>
		<div v-if="isDate" class="no-data"></div>
...
export default {
	...
	watch:{
		...
		reportType(){
		  //console.log(11111111, this.reportType)
		}
	},
	mounted () {
    let _this = this
