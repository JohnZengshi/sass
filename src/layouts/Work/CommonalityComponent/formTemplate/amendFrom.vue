                    ...
                    <template v-if="showDatas" v-for="(AllData, index) in showDatas">
                        ...
                        <tr :class="{'table-pitch-on': operate.selectedRow[index], 'table-mousemove-style': operate.mousemoveStyle[index]}" v-on:mousemove="setMousemove(AllData,index,$event)" v-on:mouseout="setMouseout">
                            ...
                            <template v-for="hostKey in getLittleClassKey(Object.keys(AllData.hostList))">
                                <template v-if='hostKey !=="barcode"'>
                                    <td>{{AllData.hostList[hostKey]}}</td>
                                    <template v-if="hostKey === 'certifiName'">
                                        <td>{{AllData.hostList.organizationName}}</td>
                                        <td>{{AllData.hostList.website}}</td>
                                        <td>{{AllData.hostList.phone}}</td>
                                        <td>{{AllData.hostList.stand1}}</td>
                                        <td>{{AllData.hostList.stand2}}</td>
                                        <td>{{AllData.hostList.stand3}}</td>
                                    </template>
                                </template>
                            </template>
                        </tr>
    ...
    methods: {
            ...
            function cbFunction () {
                seekProductClassList(requestNumber).then((response) => {
                    if (response.data.state === 200) {
                        ...
                        this.$store.dispatch('workPopupError', response.data.msg + "获取商品大小类的下拉列表");
                    }
                }, (response) => {
            ...
            function cbFunction () {
                seekProductPropertyList(requestNumber).then((response) => {
                    if (response.data.state === 200) {
                        ...
                        this.$store.dispatch('workPopupError', response.data.msg + "商品属性");
                    }
                }, (response) => {
        ...
        batchDelect () { // 确认删除
            ...
            if (this.newDatas.orderNumber) { // 如果没有单据号，就是全新添加
                ...
                this.$store.dispatch('workPopupError', "如果没有单据号，就是全新添加");
                this.deletePost();
            }
            this.deleteLocality();
        },
