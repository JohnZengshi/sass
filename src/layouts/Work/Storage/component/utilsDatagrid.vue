<template>
  <div class="utils-container">
    ...
    <span style="border-bottom:1px solid #ededed;" data-text="提交审核" @click="submitAudit" v-if="synopsiData.checkType == '1' && synopsiData.makeOrderManId == id">
      <i class="iconfont icon-tijiaoshenhe"></i>
    </span>
    <!--撤销审核 是公司职员 并且单据为审核中状态  -->
    <span data-text="撤销审核" @click="repealRevoke" v-if="curStatus.nowStatus == '2' && synopsiData.makeOrderManId == id">
      <i class="iconfont icon-chexiao"></i>
    </span>

    <span data-text="通过审核" @click="popupShow(true,'通过审核')" v-if="synopsiData.checkType == '2' && synopsiData.isCheckOrderMan == 'Y'">
      <i class="iconfont icon-tongguoshenhe"></i>
    </span>
    <span data-text="驳回审核" @click="popupShow(true,'驳回审核')" v-if="synopsiData.checkType == '2'  && synopsiData.isCheckOrderMan == 'Y'">
      <i class="iconfont icon-chexiao"></i>
    </span>
