...
<el-checkbox-group v-model="crudData.checkList"> 
        <table cellpadding="0" cellspacing="0">
            ...
            <tr class="tr-header">
                <template v-for="(item, inx) in littleClass">
                    <template v-for="(names,index) in getStateOpen(item, item.stateOpen)">
                        <th :key="index">{{names.name}}</th>
                    </template>
                    <!-- 隐藏时显示 -->
                    <template v-if='item.stateOpen === ""'>
                        <th :key="inx" :rowspan="showData.length + 1">
                            <ul class="header-state">
                                <li v-for="(n,index) in showData.length + 1" :key="index"></li>
                            </ul>
                        </th>
                    </template>
                </template> 
            </tr>
            <tr v-if="showData" v-for="(AllData,index) in showData" :key="index" @click="pitchOn(AllData,index,$event)" :class="{'table-pitch-on': operate.selectedRow[index] }">
...
export default {
  ...
  methods: {
    ...
    uploadingOne (e) { // 表格导入数据 one
        ...
        this.$http.post(url, fileData).then((response) => {
            console.log(response);
             ...
             if (response.data.state === 200) {
                 this.$store.dispatch('workPopupError', "上传表格成功");
             } else {
    ...
    batchDelect () { // 删除
        ...
        operateProductList(data).then((response) => {
            console.log("商品操作（删）")
            console.log(response)
            console.log("商品操作（删）")
            if (response.data.state === 200) {
    ...
    admendFun (initialValue, changeValue) { // 修改商品列表
        ...
        var data = {
            ...
            "unit": {
                ...
                "channel": 3,
                "OS": "web", // 手机为当前手机使用系统与版本号、WEB为浏览器系统与版本号
        ...
        this.$http.post(url, data).then((response) => {
            ...
            if (response.data.state === 200) {
                ...
                this.workProductDetail({
                 ...
                 this.$store.dispatch('workPopupError', "保存成功");
            } else {
                console.log("继续添加失败")
            }
        }, (response) => {
