<template>
    ...
    >
        ...
        <div class="receipts-list-body-left">
            <div class="orderNum">{{item.orderNum}} <span v-if="item.ifImport=='Y'" class="icon-cloud"></span></div>
            <ul class="status">
...
export default {
    ...
    methods: {
        ...
        goDetail (parm) {
            ...
            //sessionStorage.setItem('orderData',JSON.stringify(parm))
            let sDate = Date.now() +''
            this.$router.push({
                path: "/work/storage/detail" , // NewStockIn detail
                query: {
                    "orderNumber": parm.orderNum,
                    "st": SparkMD5.hash(sDate)
                    // "isRole": this.item.isRole,
                    // "isCheckOrderMan": this.item.isCheckOrderMan
                }
            })
        },
        getColor (parm, receiptStatus, item) { // 调出库位
            if (item.isLocking == 'Y') {
                return 6
            } else {
                switch (parm) {
