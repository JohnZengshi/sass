<template>
  <div class="select_preview">
    <div class="preview">
      <img @click="selectFile" :src="currSrc" id="preview_img">
    </div>
    <div class="button" style="display:none">{{useType}}</div>
...
export default {
  ...
  props: ['useType', 'userInfo', 'companyInfo', 'shopInfo', 'logo'],
  computed: {
    currSrc () {
      if (this.logo) {
        return this.logo
    ...
    currLogoSrc () {
      return this.companyInfo.logo ? this.companyInfo.logo : 'static/img/userphone.png'
    }
  },
  created () {
