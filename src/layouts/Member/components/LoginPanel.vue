  ...
  <div class="login-panel">
      <el-form v-if="isCurrent == 'pw'" label-position = "right" :model = "userInfo" :rules="rules" class="demo-form-stacked">
        ...
        <el-form-item prop="password" style="margin-top: 20px;">
          <el-input v-model="userInfo.password" type="password"  class="password-number" placeholder="请输入密码" @keyup.enter.native="loginKeyUp">
...
export default {
  ...
  methods: {
    ...
    gotoExperience (ev) {
      this.$emit('goto-experience', true)
    },
    gotoRegistry (ev) {
  ...
  created () {
      this.userInfo.phoneNumber = localStorage.getItem("UserPh");
      //this.userInfo.password = localStorage.getItem("UserPw");
