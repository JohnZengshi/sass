    ...
    <div class="add-wrap">
        ...
        <div class="add-main">
            <div class="title notice-Ope-block">
                ...
                <input maxlength="12" v-model="noticeTitle" type="text" class="operate" placeholder="输入公告标题...">
            </div>
                        ...
                        <el-checkbox-group v-model="checkList" @change="checkListChange">
                            ...
                            <li class="shop-list">店铺 <el-checkbox label="2"></el-checkbox>
                                <el-checkbox-group v-model="childList" @change="childListChange">
                                    <ul class="de-list">
                                        <li v-for="(item, index) in shopListByCo">{{item.shopName}}<el-checkbox :label="item"></el-checkbox></li>
                                    </ul>
                                </el-checkbox-group>
                            </li>
                        </el-checkbox-group>
...
import {operateNoticeCreate, operateNoticeUpdate} from 'Api/commonality/operate'
import {seekNoticeDetails} from 'Api/commonality/seek'
...
export default {
    ...
    watch: {
        noticeId (val) {
            ...
            this.receiveObject = val.receiveObject
        },
    ...
    methods: {
        ...
        noticeDetails () { // 公告详情
            ...
            seekNoticeDetails(options).then((res) => {
                if (res.data.state == 200) {
                    ...
                    this.receiveObject = this.noticeData.receiveObject
                    this.shopCkeckList = this.noticeData.shopList
                }
            }, (res) => {
            })
        },
        noticeCreate (type) {  // 创建单据
            if (this.noticeTitle == '') {
                ...
                operateNoticeUpdate(options).then((res) => {
                    console.log(res)
                    if (res.data.state == 200) {
                        if (type == 1) {
                            this.$message({
