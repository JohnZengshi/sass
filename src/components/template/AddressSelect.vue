            ...
            <div class="province-wrap" v-if="operType == 1">
                <div class="row-1 row">
                    ...
                    <div class="item-block">
                        <div @click.stop="selId(item)" :class="{active: objectId == item.objectId}" class="item" v-for="(item, index) in row1" :key="index">{{item.objectName}}</div>
                    </div>
                </div>
                <div class="row-2 row">
                    <div class="name-range">H-K</div>
                    <div class="item-block">
                        <div @click.stop="selId(item)" :class="{active: objectId == item.objectId}" class="item" v-for="(item, index) in row2" :key="index">{{item.objectName}}</div>
                    </div>
                </div>
                <div class="row-3 row">
                    <div class="name-range">L-S</div>
                    <div class="item-block">
                        <div @click.stop="selId(item)" :class="{active: objectId == item.objectId}" class="item" v-for="(item, index) in row3" :key="index">{{item.objectName}}</div>
                    </div>
                </div>
                <div class="row-4 row">
                    <div class="name-range">T-Z</div>
                    <div class="item-block">
                        <div @click.stop="selId(item)" :class="{active: objectId == item.objectId}" class="item" v-for="(item, index) in row4" :key="index">{{item.objectName}}</div>
                    </div>
                </div>
            </div>
            <div class="city-wrap" v-else-if="operType == 2">
                <div @click.stop="selId1(item)" :class="{active: cityId == item.objectId}" class="item" v-for="(item, index) in cityList" :key="index">{{item.objectName}}</div>
            </div>
            <div class="area-wrap" v-else-if="operType == 3">
                <div @click.stop="complateId(item)" :class="{active: areaId == item.objectId}" class="item" v-for="(item, index) in areaList" :key="index">{{item.objectName}}</div>
            </div>
        </div>
    </div>
</template>
