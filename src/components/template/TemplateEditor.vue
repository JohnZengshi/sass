    ...
    <div class="template-editor" @mousemove="editorMousemoveHandler" @mouseup="editorMouseupHandler">
        ...
        <div class="main-panel" :style="{top: this.menuBarHeight + 'px'}">
            <div class="right-panel">
                <TextSettingComponent ref="textSettingComponent" v-show="setting.isTextComponent" @changeComponentSetting="changeComponentSetting" @dialogVisibleFun = "dialogVisibleFun"></TextSettingComponent>
            ...
            <div class="canvas-panel">
                <div class="canvas-container" :style="canvasContainerStyle" @mousedown="canvasMousedownHandler" @mousemove.prevent="canvasMousemoveHandler" @mouseup.prevent="canvasMouseupHandler">
        ...
        <el-dialog title="字体下载"  :visible.sync="dialogVisible" size="mini" class="fontdownload-dialog">
          <p style="line-height: 25px;">
             ...
             经检测你的系统中未安装<span v-if="fonts!=''" style="color: red;">{{fonts}}</span>字体，此字体为第三方字体<br>
              如需使用该字体，请下载安装后，重启浏览器即可使用</p>
          <p class="download" style="margin-top: 20px; text-align: center;"> <a href="http://jzm-1252389350.cosgz.myqcloud.com/files/web/fonts/systemfonts.rar" ><el-button type="primary" icon="upload">下载字体包</el-button></a></p>
                            ...
                            if (component.type == 'ContainerComponent') {
                                ...
                                component.data.children = component.data.children.map(child => {
                                    ...
                                    if (child.type == 'PropertyComponent') {
                                        child.data.alignNumber = Number((child.data.alignNumber + 5).toFixed(1))
                                    }
                                    return child
                                })
                            }
        ...
        methods: {
            ...
            editorMouseupHandler(e) {
                if (this.componentDragStartData._isDraging) {
                    ...
                    let left = Number(offsetX.toFixed(1))
            ...
            canvasMousemoveHandler(e) {
                if (e.which == 1) {
                    ...
                    if (this._dragingComponent || this._resizingComponent) {
                        let dx = Number((this.endX - this.startX).toFixed(1))
                        let dy = Number((this.endY - this.startY).toFixed(1))
                        if (this._dragingComponent) {
                            this._isDraging = true
                            this.activeComponents.forEach(component => {
                                this.moveComponent(component, {top: dy, left: dx})
                            })
                        }
                        if (this._resizingComponent) {
                            ...
                            // }
                            // this._resizingComponent.data.height = Number((this._resizingComponent.data.height + dy).toFixed(1))

                            // 新
                            this._resizingComponent.data.width = Number((this._resizingComponent.data.width + dx).toFixed(1))
                            this._resizingComponent.data.height = Number((this._resizingComponent.data.height + dy).toFixed(1))
                            console.log(this._resizingComponent.data.width)
                        }
            ...
            handlePropClick(data, left = this.canvas.width / 2, top = this.canvas.height / 2) {
                let component = {
                    ...
                    data: {
                        ...
                        rotateDeg: 0,
                        width: 30 , //Math.round(160 / this.ppi * 2.54 * 7),
            ...
            moveComponent(component, pos) {
                ...
                component.data.left = Number(component.data.left.toFixed(1))
                component.data.top = Number(component.data.top.toFixed(1))
            },
            ...
            updateItemListId() {
                ...
                itemListComponents.forEach((itemListComponent,i) => {
                    let children = []
                    this.canvas.components.forEach(component => {
                    ...
                    children.forEach(child => {
                    	//console.log(i)
                        //this.moveComponentTo(child, {top: Number((itemListComponent.data.top + alignNumber - 0.5 * child.data.height).toFixed(1))})
						this.moveComponentTo(child, {top: Number((itemListComponent.data.top +  i * child.data.height).toFixed(1))})
                    })
                    //console.log(children)
