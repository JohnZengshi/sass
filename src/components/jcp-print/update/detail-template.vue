			...
			<table class="table-box">
				...
				<template v-for="(item,index) in sellList.detailList">
					<tr>
						...
						<td rowspan="2">{{item.jewelryName}}</td>
						<td>{{item.weight}}</td>
				...
				<tr>
					<td>{{sellList.afTotalNum}}件</td>
					<td>{{sellList.afTotalWeight}}g</td>
					<td>{{sellList.afTotalGoldWeight}}g</td>
					<td>{{sellList.afTotalMain}}</td>
					<td>{{sellList.afTotalDeputy}}</td>
					<td>{{sellList.afTotalPrice}}元</td>
					<td>{{sellList.afTotalCost}}元</td>
				</tr>
			</table>
		</div>
		<div class="printDate">
